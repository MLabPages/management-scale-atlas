...
- 収録データは **v0.71.0**、**66 概念**、**101 尺度**、個別の尺度使用研究 **160 件**、日本語未確認 **50 尺度**（2026-09-24）。サーバント・リーダーシップ（SL-28 / SL-7）を追加した。GitHub Pages への公開は本変更では行っていない。main へのマージもしていない。
...
- v0.71.0 はブランチ上の追加であり、`main` へは未マージ。GitHub Pages は公開していない。
- 作業開始時の `main` は origin/main の v0.70.0 と一致していた（履歴統合済み。文書初回共有コミットは `cfcf163`）。
...
- CX Scale 18（`customer-experience-scale-gahler-18`）の usage 追加と、Williams–Anderson OCBI/OCBO の usage は v0.71.0 でも見送り（HOLD）。
